            ...
            for file2 in os.listdir(seq_directory):
                ...
                if file in file2:
                    ...
                    for test1 in SeqIO.parse(input_name, 'fasta'):
                        print(test1)
                        sequence_name = file
                        file_name = "Gene" + test1.description.split(file)[1].split("_")[1].split(":")[0]
...
def mafft_alignment(seq_directory, threads = 1):
    ...
    try:
        for file in os.listdir(genes_result_s3):
            ...
            if file != ".DS_Store":
                cmd = "mafft --adjustdirection --maxiterate 1000 --globalpair --thread " + str(threads) + " " + fname + " > " + output_name
                print(cmd)
                os.system(cmd)
...
def main():
    ...
    parser.add_argument("-th", "--thread", dest='threads_num', type=int, default=1,
