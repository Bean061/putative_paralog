#### After this step, copy and paste all genes together, and remove paralogs.
import os
...
def trimal_align_2(seq_directory):
    ...
    try:
        for file in os.listdir(genes_result_s5):
            ...
            if file != genes_result_s5 + ".DS_Store":
                cmd = "trimal -in " + fname + " -out " + output_name + " -gt 0.51"
                print(cmd)
                os.system(cmd)
...
def rm_wrong_polymorphism_sites(seq_directory, outgroup_path, window_size = 20, Max_p_sites = 5):
    ### define iupac
    iupac_bases = ['m', 'r', 'w', 's', 'y', 'k', 'M', 'R', 'W', 'S', 'Y', 'K', "v", "h", "d", "b", "n", "V", "H", "D", "B"]
        ...
        if file != ".DS_Store":
            ...
            for sequence in sequences:
                ...
                for each in window(range(total_length), window_size):
                    ...
                    if len(column_position) >= Max_p_sites:
...
def replace_outgroup_with_gap(seq_directory, outgroup_path, window_size = 20):
    ### define iupac
    iupac_bases = ['m', 'r', 'w', 's', 'y', 'k', 'M', 'R', 'W', 'S', 'Y', 'K', "v", "h", "d", "b", "n", "V", "H",
                   "D", "B"]
        ...
        if file != ".DS_Store":
            ...
            for sequence in sequences:
                ...
                for each in window(range(total_length), window_size):
                    ...
                    ### if there are more than 4 polymorphic sites in 20 base pairs, select those sites positions.
                    if len(column_position_outgroup) >= 8:
...
def paralog_test(seq_directory, Hs_threshold = 0.5):
    ...
    try:
        ...
        print("s7 is not finshed yet. Please check the previous step.")
...
def main():
    parser = argparse.ArgumentParser(description="To find putative paralogs in 353 enrichment data. It requires three input components and one output component. The '-he' and '-hs' are not necessary. Dependencies: mafft and trimal installed by conda. -ifa, -ina, -iref, -io, -o are required arguments",
                                     formatter_class=argparse.RawTextHelpFormatter)

    parser.add_argument("-ifa", "--inputfastadir", dest='input_dir', type=str,
                        help='the directory where fasta files deposits, which are generated by Kate et al. iupac step')

    parser.add_argument("-ina", "--inputspna", dest='input_sp_name', type=str,
                        help='the full pathway of namelist.txt. Every line has one accession')

    parser.add_argument("-iref", "--inputref", dest='input_ref_seq', type=str,
                        help='the full pathway of reference seq. Require fasta format. The reference name should be like "SPECIESNAME-GENENAME". Please Capitalize them.')
    #### modify
    parser.add_argument("-io", "--inputoutgroup", dest='input_outgroup', type=str,
                        help='the full pathway of outgroups.txt. Every line has one accession')
    ...
    parser.add_argument("-m", "--maxpsite", dest='max_p_site', type=int, default=5,
                        help='The number of polymorphism site in each sliding window without outgroup. Default is 5.')
    ...
    if args.input_dir and args.input_sp_name and args.input_ref_seq and args.output_dir and args.input_outgroup:
        ...
        if len(os.listdir(seq_result_path)) <= 1:
            ...
            # ### s5
            # trimal_align_1(seq_result_path)
            # print("s5 finished and ready for s6")
            ...
            ### s6 trimal gap
            trimal_align_2(seq_result_path)
            ...
            replace_outgroup_with_gap(seq_result_path, os.path.realpath(args.input_outgroup), args.window_size)

            ### s8
            paralog_test(seq_result_path, Hs_threshold=args.Hs_max_value)
